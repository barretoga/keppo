...
### Modular Design
- **Separation of Concerns**: Each module handles a specific domain (auth, events, discord, scheduler)
- **Dependency Injection**: NestJS DI container for loose coupling
- **Service Layer Pattern**: Business logic separated from controllers/handlers

### Discord Integration
- **Handler Pattern**: Separate handlers for commands, modals, and select menus
- **Builder Pattern**: Reusable builders for Discord UI components
- **Validator Pattern**: Input validation for event data
- **Custom Exceptions**: Type-safe error handling

### Database Layer
- **Prisma ORM**: Type-safe database queries
- **Migration System**: Version-controlled schema changes
- **Models**: User, Event, GuildConfig

### Scheduling System
- **Cron-based**: Flexible scheduling with cron expressions
- **NestJS Schedule**: Built-in task scheduling
- **Event Polling**: Periodic checks for due events
...
1. **Database**: Migrate from SQLite to PostgreSQL
   ```env
   DATABASE_URL="postgresql://user:password@localhost:5432/keppo"
   ```
...
3. **Process Manager**: Use PM2 or similar
   ```bash
   npm install -g pm2
   pm2 start dist/main.js --name keppo
   ```
...
This project is licensed under the **UNLICENSED** license - it is private and not intended for public distribution.
...
- GitHub: [@barretoga](https://github.com/barretoga)
